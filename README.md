...
# COPYAI — PROTOCOLO MESTRE DE EXECUÇÃO (LEIA PRIMEIRO)

Este é um único projeto de reconstrução; preserve fidelidade de TODAS as rotas capturadas.

## 1. BACKEND — BLOQUEIO DE ORDEM

# COPYAI BACKEND — EXECUTION BLOCK MODE: FULL-STACK RECONSTRUCTION BACKEND CONFIDENCE: 91% Read the entire BACKEND/ folder before UI code. Build a NEW equivalent backend from observable contracts only. Never reuse source users, balances, payments, cookies, tokens, Authorization, service-role keys or private database records. Confirmed/high-confidence contracts: GET /api/contact.php. Entities: contact_php. Auth: none. Flows: contact.php. If backend is required, connect a NEW owner-controlled Supabase project, apply the supplied migration, enable/review RLS and test every confirmed contract end-to-end before completion.

Crie somente backend NOVO controlado pelo dono. Não reutilize credenciais, usuários, dados, pagamentos, cookies ou tokens da origem.

## 2. EVIDÊNCIAS

Antes de codar, leia o pacote inteiro. Inventário: 1 rota(s), 4 imagem(ns), 0 fonte(s), 0 mídia(s), 0 interação(ões).

## 3. RECONSTRUÇÃO

Use HTML/CSS, assets locais e screenshots como autoridade. Não simplifique estados, temas, embeds, motion ou controles observados.

## 4. QA

Execute matriz funcional, auditoria final e Pixel QA em todas as rotas.

## 5. ENTREGA

Só conclua com Preview executável, build testado e repositório/ZIP completo.

## COBERTURA DE BACKEND — BLOQUEIO

Complete BACKEND_CONTRACT_COVERAGE_MATRIX.md e registre prova E2E para cada contrato confirmado.

## CHECK-IN POR EVIDÊNCIA

Complete IMPLEMENTATION_CHECKIN.md e gere IMPLEMENTATION_COVERAGE.json + DELIVERY_MANIFEST.json antes de encerrar.

## MAPA AUTORITATIVO DE ROTAS
...
Revise novamente ROUTE_MAP.json e RECONSTRUCTION_ACCEPTANCE_MATRIX.md. A etapa final nunca reduz o escopo.

This project was built with [Lovable](https://lovable.dev).

## Build with Lovable

Continue developing this project in the [Lovable editor](https://lovable.dev/projects/36c2f12c-c588-4596-bd58-5004e1664228).
